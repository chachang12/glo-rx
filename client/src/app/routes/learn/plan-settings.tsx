...
import { useGetAllExams } from '@/features/learn/exams'
import {
  useGetPlan,
  useUpdatePlan,
  useDeletePlan,
...
export const PlanSettings = () => {
  ...
  const exam = exams.find((e) => e.code === examCode) ?? null
  const { data: plan, isError } = useGetPlan(examCode)
  ...
  const patchPlan = async (updates: UpdatePlanInput) => {
    if (!examCode) return
    await updateMutation.mutateAsync({ examCode, updates })
  }

  const handleRemovePlan = async () => {
    if (!examCode) return
    await deleteMutation.mutateAsync(examCode)
