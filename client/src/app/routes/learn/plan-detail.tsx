...
import { RoadmapTrack, type RoadmapDay, type RoadmapKind, type RoadmapStatus } from '@/features/learn/roadmap'
import {
  getAllExams,
...
import {
  ...
  type Plan as PlanRecord,
} from '@/features/learn/plans'
import {
  publishCustomPlan,
  unpublishCustomPlan,
  getPlanDocuments,
  type PlanDocument as DocData,
} from '@/features/learn/custom-plans'
import { listTests, type CommunityTestSummary } from '@/features/learn/tests'
...
type TestSummary = CommunityTestSummary
...
const PlanDetailLayout = ({ kind }: { kind: Kind }) => {
  ...
  useEffect(() => {
    ...
    if (kind === 'standard') {
      common.push(
        ...
        listTests({ examCode: identifier })
          .then((data) => setTests(data.tests))
          .catch(() => {}),
        getOfficialTestsForExam(identifier)
          .then(setOfficialTests)
          .catch(() => {}),
        getExamQuestions(identifier, { limit: 1 })
          .then((data) => setQuestionBankAvailable(data.length > 0))
          .catch(() => {}),
      )
    } else {
      common.push(
        getPlans()
          ...
          .catch(() => setNotFound(true)),
        getPlanDocuments(identifier).then(setDocs).catch(() => {}),
      )
    }
  ...
  const handleEnroll = async () => {
    if (kind !== 'standard' || !identifier) return
    setEnrolling(true)
    try {
      const newPlan = await createPlan({ examCode: identifier })
      setPlan(newPlan)
      setNotEnrolled(false)
      const data = await getReadiness({ kind: 'standard', identifier })
      setReadiness({ ...data, topics: normalizeTopics(data.topics) })
    } catch {
      // ignore — UI already shows enroll state
    } finally {
      setEnrolling(false)
    }
  }
  ...
  const handleGenerateRoadmap = useCallback(async () => {
    ...
    try {
      const data = await generateRoadmap({ kind, identifier })
      setRoadmap(data)
    } catch {
      // ignore
    } finally {
  ...
  const handleMarkComplete = useCallback(async (dayNumber: number) => {
    if (!identifier) return
    try {
      await markRoadmapDayComplete({ kind, identifier, dayNumber })
  ...
  const handlePublish = useCallback(async () => {
    ...
    setPublishing(true)
    try {
      const data = await publishCustomPlan(identifier)
  ...
  const handleUnpublish = useCallback(async () => {
    ...
    setPublishing(true)
    try {
      await unpublishCustomPlan(identifier)
  ...
  const refreshReadiness = useCallback(async () => {
    if (!identifier) return
    try {
      const data = await getReadiness({ kind, identifier })
      setReadiness({
        ...data,
        topics: normalizeTopics(data.topics),
        allowedQuestionTypes: data.allowedQuestionTypes ?? ['mcq'],
  ...
  const handleGenerateQuestions = useCallback(async () => {
    ...
    setDrawerError(null)
    try {
      const data = (await generateTopicQuestions({
        kind,
        identifier,
      ...
      })) as { cached?: boolean; generatedCount?: number } | undefined
      setDrawerTopic((prev) =>
        prev
          ? {
              ...prev,
      ...
      setDrawerError(err instanceof ApiError ? err.message : 'Question generation failed')
    } finally {
      setDrawerGenerating(false)
    }
  }, [drawerTopic, identifier, kind, drawerCount, drawerDifficulty, drawerInstructions, drawerTypeWeights, refreshReadiness])
        ...
        <div style={{ paddingBottom: 64 }}>
          ...
          {kind === 'custom' && docs.length > 0 && (
            <>
              ...
              <div className="row-list">
                {docs.map((doc) => (
                  <div key={doc.id} className="row-link" style={{ cursor: 'default' }}>
