...
} from '@/features/shared/user'
import {
  useGetIncomingFriendRequests,
  useAcceptFriendRequest,
  useDeclineFriendRequest,
  useSendFriendRequest,
} from '@/features/shared/friends'
...
export const Leaderboard = () => {
  const { data: entries = [], isLoading: leaderboardLoading } = useGetLeaderboard()
  const { data: requests = [], isLoading: requestsLoading } = useGetIncomingFriendRequests()
  ...
  if (leaderboardLoading || requestsLoading) return <PageLoader />
...
const AddFriendDialog = ({ onClose }: { onClose: () => void }) => {
  ...
  const { data: results = [], isFetching: searching } = useSearchUsers(trimmedQuery)
