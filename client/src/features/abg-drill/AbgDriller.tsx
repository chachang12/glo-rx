...
async function fetchVignette(
  ...
  const response = await fetch("/api/abg/vignette", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ values, imbalance, compensation }),
  });
  ...
  const prefetchNext = useCallback(
    async (nextIdx: number) => {
      ...
      try {
        ...
        const vignette = await fetchVignette(q.values, q.imbalance, q.compensation);
        setQuestions((prev) => {
          const updated = [...prev];
      ...
      <div className="min-h-screen bg-[#0f0f1a] flex items-center justify-center p-6">
        <div className="w-full max-w-md space-y-8">
          ...
          <button
            ...
            className="w-full py-3 rounded-lg font-semibold text-sm transition-all bg-[#4f8ef7] text-[#0f0f1a] hover:bg-[#4f8ef7]/90"
          >
